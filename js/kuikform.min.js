"use strict";let kuikformSnackBarWrapper,kuikformSnackBar,kuikformStatusText,isInitialized=!1,snackBarActive=!1;const serverEndpoint="https://kuikform.tanmoy.codes/api/v1/submit/";
function initPlugin(){
	isInitialized||(addElements(), kuikformSnackBarWrapper=document.querySelector(".kuikform-snackbar-wrapper"),kuikformSnackBar=document.querySelector(".kuikform-snackbar"),kuikformStatusText=document.querySelector("#kuikform-status-text"),document.querySelectorAll("[kuikform-id] > input[type=file]").forEach(function(a)
	{
		a.onchange=function(a)
		{
			a.target.files[0].size>99e6&&(alert("File size should be less than 99 MB"),this.value="")
		}
	}),document.querySelectorAll("[kuikform-id]").forEach(function(a)
	{
		a.addEventListener("submit",async function(d)
		{
			if(d.preventDefault(),console.log(a.getAttribute("kuikform-id")),a.checkValidity())
			{
				showSnackbarWithPromise(2,"Processing...");let e=await serializeArray(a);
				var f={id:a.getAttribute("kuikform-id"),data:e};
				showSnackbarWithPromise(2,"Submitting...");
				var c=await fetch("https://kuikform.tanmoy.codes/api/v1/submit/",
				{
					method:"POST",body:JSON.stringify(f),
					headers:{"Content-type":"application/json"}
				});
				console.log(c);
				var b=await c.json();
				b.success?toggleSnackbar(1,b.message):toggleSnackbar(0,b.error)
			}
		},!1)
	}))
}
function addElements(){document.head.innerHTML+=`
	<style>
		.kuikform-snackbar-wrapper{
			display: none;
			border-radius: 8px;
			background-color: rgba(0, 0, 0, 1);
			padding: 15px;
			font-family: 'Gill Sans', 'Gill Sans MT', Calibri, 'Trebuchet MS', sans-serif;
			color: white;
			-moz-user-select: none;
			-ms-user-select: none;
			user-select: none;
			position: fixed;
			top: 110vh; left: 50%;
			transform: translate(-50%, -50%);
			opacity: 0;
			transition: top .5s ease-in-out, opacity .7s ease-in-out;
		}
		
		.kuikform-snackbar{
			display: flex;
			justify-content: space-evenly;
			align-items: center;
		}
		
		.kuikform-snackbar > div{
			width: 20px;
			height: 20px;
			margin-right: 10px;
			background-size : contain;
			background-repeat: no-repeat;
		}

		.kuikform-loading > div {
			border: .2em solid transparent;
			border-top-color: currentcolor;
			border-radius: 50%;
			animation: 1s kuikform-loading-loader linear infinite;
			position: relative;
			&:before {
				content: '';
				display: block;
				width: inherit;
				height: inherit;
				position: absolute;
				top: -.2em;
				;
				left: -.2em;
				;
				border: .2em solid currentcolor;
				border-radius: 50%;
				opacity: .5;
			}
		}
		
		@keyframes kuikform-loading-loader {
			0% {
				transform: rotate(0deg);
			}
			100% {
				transform: rotate(360deg);
			}
		}
		  

		.kuikform-success > div{
			background-image : url("data:image/png;base64,");
		}

		.kuikform-failed > div{
			background-image : url("data:image/png;base64,");
		}
		
		.kuikform-snackbar > p{
			padding: 0; margin: 0;
		}

	</style>
	// `}
	
	var serializeArray=async f=>{var b={};let d=Array.prototype.slice.call(f.elements);for(let c=0;c<d.length;c++){var a=d[c];if(!(!a.name||a.disabled||["reset","submit","button"].indexOf(a.type)> -1)){if("select-multiple"===a.type)Array.prototype.slice.call(a.options).forEach(function(c){c.selected&&(b[a.name]=c.value)});else if("file"==a.type){var e=await uploadFile(a);console.log(e),b[a.name]=e;continue}else["checkbox","radio"].indexOf(a.type)> -1&&!a.checked||(b[a.name]=a.value)}}return console.log(b),b},uploadFile=async function(a){showSnackbarWithPromise(2,"Uploading...");var c=new Headers,b=new FormData;return b.append("files",a.files[0],a.files[0].name),(await (await fetch("https://file.tanmoy.codes/upload/",{method:"POST",headers:c,body:b,redirect:"follow"})).json()).files[0]};function toggleSnackbar(a,b){snackBarActive||(snackBarActive=!0,showSnackbarWithPromise(a,b).then(setTimeout(()=>{console.log("Snackbar openeed"),hideSnackbarWihPromise().then(()=>{snackBarActive=!1,console.log("Snackbar Closed")})},3e3)))}function showSnackbarWithPromise(a,b){return new Promise((c,d)=>{0==a?(kuikformSnackBar.classList.remove("kuikform-success"),kuikformSnackBar.classList.remove("kuikform-loading"),kuikformSnackBar.classList.add("kuikform-failed")):1==a?(kuikformSnackBar.classList.remove("kuikform-failed"),kuikformSnackBar.classList.remove("kuikform-loading"),kuikformSnackBar.classList.add("kuikform-success")):2==a&&(kuikformSnackBar.classList.remove("kuikform-failed"),kuikformSnackBar.classList.remove("kuikform-success"),kuikformSnackBar.classList.add("kuikform-loading")),kuikformStatusText.innerHTML=b,kuikformSnackBarWrapper.style.display="inline-block",setTimeout(()=>{kuikformSnackBarWrapper.style.opacity=1,kuikformSnackBarWrapper.style.top="90vh",c()},10)})}function hideSnackbarWihPromise(){return new Promise((a,b)=>{kuikformSnackBarWrapper.style.opacity=0,kuikformSnackBarWrapper.style.top="110vh",setTimeout(()=>{kuikformSnackBarWrapper.style.display="none",a()},500)})}window.onload=()=>{initPlugin()}